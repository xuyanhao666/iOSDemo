...
#define kDefaultTabWidth  150  //根据(屏幕的宽度 / tab的宽度) 来设置出现几个Tab
...
- (void)reloadData {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
        ...
        if (_tabWidth == 0) {
            ...
            _tabWidth = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:_tabWidth];
    ...
    for (int i = 0; i < _tabCount; i++) {
        ...
        _tabView = [self tabViewAtIndex:i];
        ...
        [_tabsView addSubview:_tabView];
        
        contentSizeWidth += _tabView.frame.size.width;
        
        // To capture tap events
        UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapGesture:)];
        [_tabView addGestureRecognizer:tapGestureRecognizer];
    }
