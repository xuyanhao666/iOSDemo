...
static const CGFloat CSToastVerticalPadding     = 12.5;
...
static const CGFloat CSToastShadowRadius        = 10;
...
- (void)makeToastActivity:(id)position andText:(NSString *)text{
    ...
    UIActivityIndicatorView *activityIndicatorView = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
    activityIndicatorView.center = CGPointMake(activityView.bounds.size.width / 2, activityView.bounds.size.height / 2 - 17);
    [activityView addSubview:activityIndicatorView];
    [activityIndicatorView startAnimating];
    // associate the activity view with self
    objc_setAssociatedObject (self, &CSToastActivityViewKey, activityView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, activityView.bounds.size.height - 40, 100, 40)];
    textLabel.text = text;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2))  - 65);
}
